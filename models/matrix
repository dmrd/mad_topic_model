...
//set b to be a subarray of a, starting at index start
template<class T> inline void
subarr(const D1Array<T> &a, uint_32t start, D1Array<T> &b)
{
    assert( start >= 0);
    assert(start+b.size() <= a.size() )
    const T * const ad = a.data();
    T *bd = b.data();
...
template<class T> inline void
hadamard(const D1Array<T> &a, D1Array<T> &b,D1Array<T> &c)
{
    assert (a.dim_equal(b));
