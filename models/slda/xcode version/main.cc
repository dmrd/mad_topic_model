...
string Env::prefix = "";
Logger::Level Env::level = Logger::DBUG;
...
main(int argc, char **argv)
{
  ...
  while (i <= argc - 1) {
   ...
   // m = 13481;
    //n = 130873;
  Env env(n, m, k, k2, fname, nmi, ground_truth_fname, rfreq,
  ...
  if (ratings.read(fname.c_str()) < 0) {
    ...
    fprintf(stdout, "params %d, %d, %d", env.m,env.n,env.k);
  
    /**
  if (explore) {
    ratings.load_movies_metadata(fname.c_str());
    ZIPF zipf(env, ratings);
