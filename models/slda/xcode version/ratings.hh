...
public:
  ...
  const SparseSet &moviesSet() const {return _moviesSet;}
  SparseSet &moviesSet() {return _moviesSet;}

  const SparseSet &usersSet() const {return _usersSet;}
  SparseSet &usersSet() {return _usersSet;}
 ...
 // void copyToSet();
  bool contains (uint32_t a, uint32_t b);
...
private:
  ...
  SparseSet _moviesSet;
  SparseSet _usersSet;
    D1Array<std::set<uint32_t> > _usersSet2;
...
Ratings::copyToSet()
{
  for (uint32_t n = 0; n < _movies.size(); ++n)
  {
    const vector<uint32_t> *movies = _movies[n];
      std::tr1::unordered_set<uint32_t> s;
      
    
   for (uint32_t j = 0; j < movies->size(); ++j)
    {
        uint32_t mov = (*movies)[j];
        s.insert(mov);
        _usersSet2[n].insert(mov);
    }
      _usersSet[n]=&s;
  }
  for (uint32_t m = 0; m < _env.m; ++m)
  {
      const vector<uint32_t> * users = _users[m];
      std::tr1::unordered_set<uint32_t> s;
...
inline bool
Ratings::contains(uint32_t a, uint32_t b)
{
  
if (_usersSet2[a].count(b)>0)
    return true;
  return false;
}
**/
