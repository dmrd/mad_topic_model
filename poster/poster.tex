...
The Multivalence Authorship Detection (MAD) Topic Model extends Latent Dirichlet Allocation\cite{LDA}to indentify authorship in documents with many separate types ("multivalent") count features. MAD is ``doubly supervised'' - it includes a multi-class logistic regression as in\cite{SLDA} - and also fits per-author dirichlet distributions for each feature type. We test the MAD Topic Model on many real world corpora using a variety of n-gram features, ranging from parts of speech, syllable stress, and sequences of word lengths. 
\end{alertblock} 
...
\begin{figure}[h!]
  ...
  \tikzstyle{box}=[rectangle, draw=black!100]
    ...
    \node[main] (eta) [above=of y, xshift = 26mm,yshift=-2mm]{$\eta$};
    \path (alpha) edge [connect] (theta)
          ...
          (eta) edge [connect] (y)
          (w) edge [connect] (y);
    \node[rectangle, inner sep=8mm, fit= (z) (w),label=below right:N, yshift = 8mm, xshift=4mm] {};
	...
	\node[rectangle, inner sep=16mm, fit= (theta) (z) (w) (y),label=below:M, yshift = 3mm, xshift=12.5mm] {};
	\node[rectangle, inner sep=16mm, draw=black!100, fit = (theta) (z) (y) (w), yshift = -12mm, xshift=16] {};
	\node[rectangle, inner sep=20mm, fit= (alpha) (theta) (z) (w) (y),label=below left:A, yshift = 5mm, xshift=-30mm] {};
	\node[rectangle, inner sep=20mm, draw=black!100, fit = (alpha) (theta) (z) (w) (y), yshift = -13mm, xshift=16] {};
	\node[rectangle, inner sep=28mm, fit = (lambda) (beta) (alpha) (theta) (z) (w), label=above right:T, yshift = -30mm, xshift=50] {};
  ...
  \end{tikzpicture}
  \caption{MAD Topic Model Graphical Model}
\end{figure}
